...
const ScheduleTImePopup: FC<ScheduleTImePopupProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex max-w-[525px] flex-col gap-6 border-[1px] border-[#fff5e5] bg-[#fff5e5]">
        <DialogHeader>
              ...
              {dates?.map((date) => {
                return (
                  <div
                    key={date?.id}
                    className={cn(
                      "flex w-fit flex-col items-start justify-center gap-3 border-[2px] border-[#282828] px-4 py-3",
                      select === date.id && "border-primary",
                    )}
                    onClick={() => {
                      setSelect(date.id);
